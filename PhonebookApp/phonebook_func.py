...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # ensure user provides both names
        ...
        with conn:
            cursor = conn.cursor()
            # Check the database for existance of the fullname, if so, we will alert user and disregard requrest
            cursor.execute("""SELECT COUNT(col_fullname) \
                              FROM tbl_phonebook \
                              WHERE col_fullname = '{}'""".format(var_fullname)) #, (var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if thhis is 0, then this user is not in the database and we can add it
    ...
    with conn:
        ...
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmation", "All information associated with, ({})"
                                             ...
                                             "\n\nProceed with the deletion requrest?".format(var_select))
            if confirm:
                conn = sqlite3.connect('phonebook.db')
                with conn:
                    cursor = conn.cursor()
            ...
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the databsase and " 
                                            ...
                                            "first before deleting ({}).".format(var_select, var_select))
    conn.close()
...
def onUpdate(self):
    ...
    var_email = self.txtemail.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure data is present
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cur = conn.cursor()
            # couontrecords to see if the user's changes are alareadyh in
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.dursor()
                        cursor.execute("""UPDATE tbl_phonebook SET col_phone = '{0}', col_email = '{1}' \
                ...
                messagebox.showionfo("No changes detected","Box ({}) and ({}) \nalready exist in the database for \
